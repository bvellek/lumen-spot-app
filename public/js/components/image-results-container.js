  ...
  render() {
    ...
    if (images != null) {
      imageList = images.photos.map((pic, i) => {
        return (
          <Image altText={pic.name} aperture={pic.aperture} iso={pic.iso} ss={pic.ss} imageURL={pic.image_url} imageLink={`https://500px.com${pic.url}`} imageLinkTarget={`_${pic.user.id}`} photog={`${(pic.user.firstname).toUpperCase()} ${(pic.user.lastname).toUpperCase()}`} key={i} />
        );
      });
    } else {
        imageList = (
